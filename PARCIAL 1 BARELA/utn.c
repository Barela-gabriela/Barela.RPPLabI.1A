...
int getValidString(char requestMessage[],char errorMessage[], char errorMessageLenght[],char input[], int lowLimit,int maxLenght)
{
    ...
    while(1)
    {
        if (!getStringLetras(requestMessage,buffer))       ///El segundo error es por si me paso del TAM de la funcion
        {
            printf ("%s\n",errorMessage);                  ///lowlimit, la cantidad de letras
            continue;
        }
...
int getValidInt(char requestMessage[],char errorMessage[], int lowLimit, int hiLimit,int* input)
{
    ...
    while(1)
    {
        if (!getStringNumeros(requestMessage,auxStr))           ///Numero mas bajo que cargar low limit
        {
            printf ("ERROR, solo ingrese numeros \n");          ///imput direccion de memoria, el numero que le paso
            continue;
